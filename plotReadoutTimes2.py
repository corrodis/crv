...
if True:
    files_ = ["syncTestGain28And10FPG1Port1And3_1000.csv", "syncTestGain10And28FPG1Port1And3_1000.csv", "syncTestGain28FPG1Port1And3_1000.csv", "syncTestGain31Th10And10FPG1Port1And3_1000_2.csv", "syncTestGain31Th10And10FPG1Port1And3_1000.csv", "syncTestGain31Th8And28FPG1Port1And3_1000.csv"]


    skip  = [50, 50, 50, 50, 50, 160]
    data_ = []
...
if False:
    ...
    for k, d in enumerate(data_):
      if k not in  [4]:
        if k in [0]:
          plt.plot(d[4,:] +0.1,     np.array(d[2,:])*1e-3,'v', label=f"# hits of FEB (1)", color=sns.color_palette()[0])
          plt.plot(d[3,:] -0.1,      np.array(d[2,:])*1e-3,'x', label=f"# hits of FEB (3)", color=sns.color_palette()[3]) 
          plt.plot(d[3,:]+d[4,:],                          np.array(d[2,:])*1e-3,'v', label=f"sum of hits from both FEBs", color='k')#sns.color_palette()[3]) 
        else:
          plt.plot(d[4,:] +0.1,        np.array(d[2,:])*1e-3,'v', color=sns.color_palette()[0])
          plt.plot(d[3,:] + d[4,:],                          np.array(d[2,:])*1e-3,'x', color='k')#sns.color_palette()[2]) 
          plt.plot(d[3,:] -0.1,        np.array(d[2,:])*1e-3,'.', color=sns.color_palette()[3])
...
if False:
    ...
    skip  = [0, 0, 0, 0, 0, 5,5, 10, 10, 10, 10,10]
    data_ = []
    for k, f in enumerate(files_):
        data_.append(np.loadtxt("data/"+f)[:,skip[k]:])
    nn1_    = np.array([])
    ...
    for k, d in enumerate(data_):
      if k in [11]:
        if k in [0]:
          plt.plot(np.minimum(d[4,:], d[3,:]) +0.1,        np.array(d[2,:])*1e-3,'.', label=f"# hits of less active FEB", color=sns.color_palette()[0])
          plt.plot(np.maximum(d[4,:], d[3,:]) -0.1,        np.array(d[2,:])*1e-3,'.', label=f"# hits of more active FEB", color=sns.color_palette()[1]) 
          plt.plot(d[3,:]+d[4,:],                          np.array(d[2,:])*1e-3,'.', label=f"sum of hits from both FEBs", color=sns.color_palette()[2]) 
        else:
          plt.plot(np.minimum(d[4,:], d[3,:]) +0.1,        np.array(d[2,:])*1e-3,'.', color=sns.color_palette()[0])
          plt.plot(d[3,:]+d[4,:],                          np.array(d[2,:])*1e-3,'.', color=sns.color_palette()[2]) 
          plt.plot(np.maximum(d[4,:], d[3,:]) -0.1,        np.array(d[2,:])*1e-3,'.', color=sns.color_palette()[1])
